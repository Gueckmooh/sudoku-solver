    ...
    for i in range(N**2):
        for j in range(N**2):
            if (i, j) in set(core):
                ...
                out += " %d " % grid[i][j] if grid[i][j] != 0 else "   "
...
def read_grid_from_command():
    ...
    N = int(
        raw_input(
            "Give the size of the grid (for 9x9 grid give 3 (3**2 = 9)): "
        )
    )
    lines = []
...
def generate_z3_grid():
    ...
    grid = [
        [
            [Int("x_%d_%d_%d" % (i, j, k)) for k in range(N**2)]
            for j in range(N**2)
        ]
        for i in range(N**2)
    ]
...
for i in z3_grid:
    for j in i:
        for k in j:
            val = k >= 0
            if use_unsat_core:
                solver.assert_and_track(val, f"pos{j}{k}")
            else:
                solver.add(val)
...
vprint(
    "Asserting that there must be only one iteration of each values in "
    "1..%d per line..\n" % (N**2)
)
